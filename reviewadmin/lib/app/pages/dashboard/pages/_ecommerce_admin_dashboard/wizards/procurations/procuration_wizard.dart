...
import 'package:jatai_etadmin/app/core/helpers/fuctions/_overlay_helper.dart';
...
import 'package:jatai_etadmin/app/pages/dashboard/pages/_ecommerce_admin_dashboard/wizards/procurations/finalization_view.dart';
...
class _ProcurationFormWizardState extends State<ProcurationFormWizard> {
  ...
  void saveprocuration(AppThemeProvider wizardState) async {
    ...
    await createprocuration(payload).then((res) async {
      if (res.status == true) {
        ...
        wizardState.inventoryLocatairesSortant = [...procuration.exitenants!];
        ...
        Jks.paymentState.paymentMade = "";
        Jks.proccurationState.fetchProccurations(refresh: true, type: "all");

        for (final tenant
            in getAllProcurationAuthors(Jks.wizardState, ownerOnly: true)) {
  ...
  void _nextStep({String? from}) async {
    ...
    if (wizardState.currentStep == WizardStep.values[0]) {
      ...
      bool isConnectedUserOwner = wizardState.inventoryProprietaires
          .any((owner) => owner.email == userEmail);
    ...
    if (wizardState.currentStep == WizardStep.signatures) {
      bool _validateSignatures() {
        for (final aythor
            in getAllProcurationAuthors(wizardState, ownerOnly: true)) {
          final tenantKey = '${aythor.id}';
          var labelname = authorname(aythor);

          var isOwner = wizardState.inventoryProprietaires
              .any((owner) => owner.id == aythor.id);
          ...
          final completedCount =
              actionsCompleted.where((action) => action).length;
      ...
      if (unsignedOwners.isNotEmpty) {
        String unsignedNames =
            unsignedOwners.join(unsignedOwners.length == 2 ? " et " : ", ");
        ...
        builder: (context) => Wrap(
          children: [
            ListTile(
              ...
              subtitle: Text(
                'Êtes-vous sûr de vouloir signer la proccuration ? Cette action est irréversible et vous ne pourrez pas modifier les informations une fois signé.'
                    .tr,
      ...
      for (final tenant in getAllProcurationAuthors(wizardState)) {
        if (Jks.griffes!["${tenant.id}"] != null) {
          payload["tenantSignatures"]["${tenant.id}"] =
              base64Encode(Jks.griffes!["${tenant.id}"]);
        }
      }
      await griffeprocuration(payload).then((res) async {
        if (res.status == true) {
          ...
          await showFullScreenSuccessDialog(
            context,
            title:
                'Votre Proccuration est maintenant dans votre espace personnel.',
            message:
  ...
  Widget build(BuildContext context) {
    ...
    wizardState = context.watch<AppThemeProvider>();
    bool canLeaveSafely = wizardState.currentStep == WizardStep.values[7] ||
    ...
    return Consumer<AppThemeProvider>(
      builder: (context, lang, child) {
        ...
        Jks.imperativeActions["beforeLeaveProccuration"] = canLeaveSafely
            ? null
            : () {
                if (wizardState.currentStep != WizardStep.values[0]) {
            ...
            body: Column(
              children: [
                ...
                LinearProgressIndicator(
                  value: (_currentStep.index + 1) / (WizardStep.values.length),
                ),
                      ...
                      if (_currentStep != WizardStep.confirmation)
                        ElevatedButton.icon(
                          ...
                          icon: wizardState.loading
                              ...
                              : null,
                          label: Text(_currentStep == WizardStep.values[5]
                              ? _lang.save
                              : _lang.next),
                        ),
                      if (_currentStep == WizardStep.confirmation)
