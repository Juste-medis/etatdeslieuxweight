                      ...
                      child: Column(
                        children: reviewstate.reviews
                            .map(
                              (review) => ReviewCard(
                                from: "dashboard",
                                index: reviewstate.reviews.indexOf(review),
                                review: review,
                                onTap: () => handleReviewTap(context, review),
                              ),
                            )
                            .toList(),
                      )))
...
Widget buildReviewEmptyState(ThemeData theme, {Map? filtering}) {
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        Text(
          "Aucun état des lieux trouvé".tr,
          style: theme.textTheme.titleMedium,
          textAlign: TextAlign.center,
        ),
        const SizedBox(height: 8),
        Text(
          "Aucun état des lieux ${(filtering != null && filtering.isNotEmpty) ? "ne correspond à vos critères de recherche" : "n'a encore été créé"}"
              .tr,
      ...
      children: [
        ...
        ListTile(
            ...
            onTap: () async {
              ...
              if (confirmed ?? false) {
                final deletion = await reviewState.deleteTheReview(
                  review,
                );
                if (deletion) {
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (DateTime.now().difference(review.createdAt!).inMinutes < 30)
                SizedBox(
                    height: 22,
...
Color _getStatusColor(ThemeData theme, String? status) {
  switch ((status ?? "draft").toLowerCase()) {
    ...
    case 'signing':
      return Colors.orange;
    case 'draft':
      return theme.brightness == Brightness.dark ? Colors.white70 : Colors.grey;
    default:
